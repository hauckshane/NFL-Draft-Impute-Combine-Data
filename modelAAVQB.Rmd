...
```{r}
fbrefespnshort <- read_csv("data/fbrefespnshort.csv") %>% select(-1) %>%
  mutate(player = trimws(gsub("\\  ", " ", player))) %>%
  mutate(GRADE = if_else(player == "robert griffin", 97, GRADE),
         `PK(OVR)` = if_else(player == "robert griffin", '2(2)', `PK(OVR)`),
         `OVR RANK` = if_else(player == "robert griffin", 2, `OVR RANK`))
...
careerPlayoffs <- read_csv("data/careerPlayoffs.csv") %>%
  group_by(Player) %>% 
...
#CREATE AAV
careerAvs <- read_csv("data/careerAvs.csv") %>%
  group_by(Player) %>% 
  mutate(firstTeam = Team[which.min(Season)]) %>%
  filter(Team == firstTeam) %>%
...
```{r}
# NEW ARTICLE SENTIMENT ANALYSIS
...
```{r}
fillin_average <- read_csv("data/fillin_average.csv") %>% select(-1, -drafted, -AV, -position)

heightweightcollegestats <- read_csv("data/heightweightcollegestats.csv") %>% select(-1)

combinepredicted <- read_csv("data/combinepredicted.csv") %>% select(-1)

combinepredictedcollege <- read_csv("data/combinepredictedcollege.csv") %>% select(-1)
```
...
fillin_average_full <- left_join(fbrefespnshort, fillin_average) %>% 
  ...
  mutate(across(.cols = c(31:38), ~replace(., is.na(.), 0)))  %>% select(-c(39:42)) %>%
  mutate(
...
heightweightcollegestats_full <- left_join(fbrefespnshort, heightweightcollegestats) %>% 
  ...
  left_join(sentiment) %>%
  mutate(across(.cols = c(31:42), ~replace(., is.na(.), 0))) %>%
  mutate(
    GRADE = if_else(is.na(GRADE), min(GRADE, na.rm = T) - 1, GRADE),
    `OVR RANK` = if_else(is.na(`OVR RANK`), max(`OVR RANK`, na.rm = TRUE) + 1, `OVR RANK`),
    projRND = ceiling(`OVR RANK` / 32),
...
```{r}
ggplot(fillin_average_full, aes(x = GRADE, y = AAV)) +
  geom_point()
```
...
FIA <- fillin_average_full %>% 
  ...
  mutate(Grade95 = as.factor(if_else(GRADE >= 95, 1, 0)),
         ...
         NotScouted = as.factor(if_else(GRADE == 30, 1, 0))) %>%
  mutate(across(.cols = c(Cmp, Att, Yds, TD, Int, RushYds, RushAtt, RushTD), ~./G, .names = "{.col}_pg"),
         TD_pa = TD / Att,
         Int_pa = Int / Att,
         Att_pg, Att / G) %>%
  mutate(combinescore = (rank(forty) + rank(desc(vertical)) + rank(desc(broad_jump)) + rank(three_cone) + rank(shuttle))/5) %>%
  mutate(across(.cols = c(forty:shuttle), ~rank(.),.names = "{.col}_rank")) %>%
  mutate(across(.cols = c(forty:shuttle), ~scale(.), .names = "{.col}_scale")) %>% 
  rename(OVR_RANK = `OVR RANK`)

FIA <- FIA %>% select(AAV, GRADE, forty:shuttle, Rate, AYPA, OVR_RANK, Grade95, Grade60, top15pick, top32pick, Round1, OSRS, SRS, NotScouted, TD_pa, Int_pa, Pct,  weighted_pos_sentiment,TotalStories)
...
while (i <= 10) {
  ...
  FIA_train_gam_cv <- gam(as.formula(paste("AAV ~ s(GRADE) + s(OVR_RANK) + s(Rate) + s(SRS) + s(OSRS) + s(AYPA) +
                                           s(TD_pa) + s(Pct) + s(Int_pa)")), 
  ...
  # Refit the model using the optimal lambda value
  FIA_test_gam_cv <- gam(as.formula(paste("AAV ~ s(GRADE) + s(OVR_RANK) + s(Rate) + s(SRS) + s(OSRS) + s(AYPA) + 
                                          s(TD_pa) + s(Pct) + s(Int_pa)")), 
  ...
  FIA_spline <- left_join(FIA, rbind(FIA_train_spline, FIA_test_spline)) %>% 
    ...
    select(-GRADE, -OVR_RANK, -Rate, -SRS, - OSRS, -AYPA, -TD_pa, -Int_pa, -Pct,
...
ggplot(data = FIA_lasso_AV_preds, aes(x = actual, y = s0)) +
  ...
  xlab("Actual AAV") +
  ylab("Predicted AAV") +
  ggtitle("Actual AAAV vs. Predicted AAV")
...
HWCS <- HWCS %>% select(AAV, GRADE, forty:shuttle, Rate, AYPA, OVR_RANK, Grade95, Grade60, top15pick, top32pick, Round1, OSRS, SRS, NotScouted, TD_pa, Int_pa, Pct,  weighted_pos_sentiment,TotalStories)
...
while (i <= 10) {
  ...
  # Refit the model using the optimal lambda value
  HWCS_train_gam_cv <- gam(as.formula(paste("AAV ~ s(GRADE) + s(OVR_RANK) + s(Rate) + s(SRS) + s(OSRS) + s(AYPA) +
                                           s(TD_pa) + s(Pct) + s(Int_pa)")), 
  ...
  # Refit the model using the optimal lambda value
  HWCS_test_gam_cv <- gam(as.formula(paste("AAV ~ s(GRADE) + s(OVR_RANK) + s(Rate) + s(SRS) + s(OSRS) + s(AYPA) + 
                                          s(TD_pa) + s(Pct) + s(Int_pa)")), 
...
NFLC <- NFLC %>% select(AAV, GRADE, forty:shuttle, Rate, AYPA, OVR_RANK, Grade95, Grade60, top15pick, top32pick, Round1, OSRS, SRS, NotScouted, TD_pa, Int_pa, Pct,  weighted_pos_sentiment,TotalStories)
...
while (i <= 10) {
  ...
  # Refit the model using the optimal lambda value
  NFLC_train_gam_cv <- gam(as.formula(paste("AAV ~ s(GRADE) + s(OVR_RANK) + s(Rate) + s(SRS) + s(OSRS) + s(AYPA) +
                                           s(TD_pa) + s(Pct) + s(Int_pa)")), 
  ...
  # Refit the model using the optimal lambda value
  NFLC_test_gam_cv <- gam(as.formula(paste("AAV ~ s(GRADE) + s(OVR_RANK) + s(Rate) + s(SRS) + s(OSRS) + s(AYPA) + 
                                          s(TD_pa) + s(Pct) + s(Int_pa)")), 
...
ggplot(data = NFLC_lasso_AV_preds, aes(x = actual, y = s0)) +
  ...
  xlab("Actual AAV") +
  ylab("Predicted AAV") +
  ggtitle("Actual AAAV vs. Predicted AAAV (NFLC Lasso)")
...
ALL <- ALL %>% select(AAV, GRADE, forty:shuttle, Rate, AYPA, OVR_RANK, Grade95, Grade60, top15pick, top32pick, Round1, OSRS, SRS, NotScouted, TD_pa, Int_pa, Pct, TotalStories, weighted_pos_sentiment)
...
while (i <= 10) {
  ...
  # Refit the model using the optimal lambda value
  ALL_train_gam_cv <- gam(as.formula(paste("AAV ~ s(GRADE) + s(OVR_RANK) + s(Rate) + s(SRS) + s(OSRS) + s(AYPA) +
                                           s(TD_pa) + s(Pct) + s(Int_pa)")), 
  ...
  # Refit the model using the optimal lambda value
  ALL_test_gam_cv <- gam(as.formula(paste("AAV ~ s(GRADE) + s(OVR_RANK) + s(Rate) + s(SRS) + s(OSRS) + s(AYPA) + 
                                          s(TD_pa) + s(Pct) + s(Int_pa)")), 
...
```{r}
ggplot(fillin_average_full, aes(x = Rate, y = AAV)) +
  geom_point()
```


```{r}
cormatirx <- cor(fillin_average_full[,unlist(lapply(fillin_average_full, is.numeric))])
cor_order <- sort(cormatirx[, "AV"], decreasing = TRUE)
```
...
for (i in seq_along(span_seq)) {
  span <- span_seq[i]
  loess_model <- loess(AAV ~ OVR, fillin_average_full %>% mutate(OVR = if_else(player == "ben roethlisberger", 11, OVR)) %>%
  mutate(OVR = if_else(player == "jason campbell", 25, OVR),
         OVR = if_else(player == "alex smith", 1, OVR),
         OVR = if_else(player == "jamarcus russell", 1, OVR)), 
  span = span)
  cv_results[i] <- mean((fillin_average_full$AAV - predict(loess_model))^2, na.rm = TRUE)
}
...
```{r}
loess(AAV ~ OVR, fillin_average_full %>% mutate(OVR = if_else(player == "ben roethlisberger", 11, OVR)) %>%
  mutate(OVR = if_else(player == "jason campbell", 25, OVR),
         OVR = if_else(player == "alex smith", 1, OVR),
         OVR = if_else(player == "jamarcus russell", 1, OVR)), span = optimal_span)


ggplot(fillin_average_full %>% mutate(OVR = if_else(player == "ben roethlisberger", 11, OVR)) %>%
  mutate(OVR = if_else(player == "jason campbell", 25, OVR),
         OVR = if_else(player == "alex smith", 1, OVR),
         OVR = if_else(player == "jamarcus russell", 1, OVR)), aes(x = OVR, y = AAV)) +
  geom_point(size = 2) + 
  geom_smooth(method = "loess", se = FALSE, color = "red", linewidth = 2, span = optimal_span) + 
  labs(x = "OVERALL PICK NUMBER", y = "AAV", title = "NFL LOESS of QBs") +
